        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                return render(request, 'todoapp/signupuser.html',
                              {'form': UserCreationForm(), 'error': "That username has already been taken. Please chose a new username"})
        else:
...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'todoapp/loginuser.html', {'form': AuthenticationForm(), 'error': 'Username and Password did not march'})
        else:
            login(request, user)
            return redirect('currenttodos')
...
def viewtodo(request, todo_pk):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request, 'todoapp/viewtodo.html', {'todo': todo, 'form': form, 'error': 'Bad data passed in. Try again'})
